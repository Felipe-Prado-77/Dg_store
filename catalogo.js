(function () {
  ...
  function buildCard(product) {
    ...
    const price = document.createElement('strong'); price.className = 'product-price'; price.textContent = window.DGData.money(product.price);
    ...
    cart.addEventListener('click', () => window.DGStore.addToCart({ id: product.id, name: product.name, price: product.price, image: product.images[0] || '', quantity: 1 }));
    actions.append(details, cart); body.append(label, name, description, price, actions); card.append(imageLink, body); return card;
  }
