(function () {
  ...
  function renderSpecifications() {
    ...
    const specs = product.specifications.length ? product.specifications : [
  ...
  function relatedCard(item) {
    ...
    const info = document.createElement('div'); const label = document.createElement('span'); label.textContent = item.category === 'relogios' ? 'RELÓGIOS' : 'PRODUTOS'; const title = document.createElement('h3'); const link = document.createElement('a'); link.href = imageLink.href; link.textContent = item.name; title.appendChild(link); const price = document.createElement('strong'); price.textContent = window.DGData.money(item.price); info.append(label, title, price); card.append(imageLink, info); return card;
  }
  ...
  function render() {
    ...
    document.getElementById('productDescription').textContent = product.description; document.getElementById('fullDescription').textContent = product.description; document.getElementById('productPrice').textContent = window.DGData.money(product.price);
    const stock = document.getElementById('stockMessage'); stock.classList.toggle('out', product.stock < 1); stock.querySelector('span').textContent = product.stock > 0 ? `${product.stock} unidade(s) disponível(is)` : 'Produto esgotado';
    quantityInput.max = String(Math.max(1, product.stock)); document.getElementById('addProductToCart').disabled = product.stock < 1;
    renderGallery(); renderSpecifications(); loading.hidden = true; content.hidden = false; renderRelated();
  }

  function safeQuantity() { const value = Math.max(1, Math.min(product?.stock || 1, Number(quantityInput.value) || 1)); quantityInput.value = value; return value; }
  document.getElementById('quantityMinus').addEventListener('click', () => { quantityInput.value = Math.max(1, safeQuantity() - 1); });
  document.getElementById('quantityPlus').addEventListener('click', () => { quantityInput.value = Math.min(product?.stock || 1, safeQuantity() + 1); }); quantityInput.addEventListener('change', safeQuantity);
  document.getElementById('addProductToCart').addEventListener('click', () => { if (!product || product.stock < 1) return; window.DGStore.addToCart({ id: product.id, name: product.name, price: product.price, image: product.images[0] || '', quantity: safeQuantity() }); });
